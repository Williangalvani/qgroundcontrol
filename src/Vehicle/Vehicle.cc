...
void Vehicle::_initializeCsv()
{
    ...
    for (auto groupName: factGroupNames()) {
        for(auto factName: getFactGroup(groupName)->factNames()){
...
void Vehicle::_writeCsvLine()
{
    ...
    for (auto fact : factNames()) {
        allFactValues << getFact(fact)->cookedValueString();
    }
