...
void FirmwarePlugin::_versionFileDownloadFinished(QString& remoteFile, QString& localFile, Vehicle* vehicle)
{
    ...
    QRegularExpressionMatch match = QRegularExpression(_versionRegex()).match(versionContents);
        ...
        )
    {
        ...
        const static QString message = tr("Vehicle firmware is not on latest stable! Running %1, latest stable is %2");
        qgcApp()->showMessage(message.arg(currentVersion, version));
    }
}
