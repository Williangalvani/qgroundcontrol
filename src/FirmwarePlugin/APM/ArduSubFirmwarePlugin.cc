    ...
    _infoFactGroup(this)
{
    ...
    factRenameMap[QStringLiteral("altitudeRelative")] = QStringLiteral("Depth");
    factRenameMap[QStringLiteral("flightTime")] = QStringLiteral("Dive Time");
}
...
void ArduSubFirmwarePlugin::adjustMetaData(MAV_TYPE vehicleType, FactMetaData* metaData)
{
    ...
    if (factRenameMap.contains(metaData->name())) {
        metaData->setShortDescription(QString(factRenameMap[metaData->name()]));
    }
}
