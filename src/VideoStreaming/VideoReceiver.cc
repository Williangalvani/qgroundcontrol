...
VideoReceiver::_startAudio()
{
    ...
    QString pipeline = QStringLiteral("udpsrc port=%1 ! application/x-rtp, media=audio, clock-rate=44100, encoding-name=L16, encoding-params=1, channels=1, payload=96 ! rtpL16depay name=rtp0 ! audioconvert ! volume volume=1.0 ! queue ! autoaudiosink sync=false");
    qCDebug(VideoReceiverLog) << "Opening audio pipeline: " << pipeline.arg(port);
    _audioPipeline = gst_parse_launch((char*)pipeline.arg(port).toLocal8Bit().data() , &error);
    if (_audioPipeline) {
        gst_element_set_state(_audioPipeline, GST_STATE_PLAYING);

        _gstVolume = gst_bin_get_by_name(GST_BIN(_audioPipeline), "volume0");
    }
    qCDebug(VideoReceiverLog) << error;
}

void
VideoReceiver::updateAudioPort()
{
    bool changeStatus;
    uint32_t port = _videoSettings->audioUdpPort()->rawValue().toUInt();
